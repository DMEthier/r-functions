...
bscdata.sqlSave2 <- function(tablename, df, fields=names(df), textfile=NA, append=T, savetable=F, datasource=NA, batchSize=1000) {
  ...
  for (i in 1:length(df[,1])) {
     ...
     sql1 <- paste(sql1, paste("INSERT INTO ", tablename, " (", fieldnames, ") VALUES (", sep=""), sep="\n")
     ...
     if (savetable & i %% batchSize == 0) {
        j <- sqlQuery(channel, sql1)
        if (length(j) > 0) {
          errorcnt <- errorcnt + 1
          if (errorcnt <= 50) warning(j)
        }
        sql1 <- ""
     }
